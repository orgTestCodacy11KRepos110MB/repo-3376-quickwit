...
use quickwit_config::{KafkaSourceParams, PulsarSourceParams};
...
pub(super) async fn check_connectivity(params: PulsarSourceParams) -> anyhow::Result<()> {
}
...
mod kafka_broker_tests {
    ...
    async fn test_kafka_source_process_message() {
        ...
        let mut kafka_source = PulsarSource::try_new(ctx, params, ignored_checkpoint)
            .await
            .unwrap();
